...
export function buildQuickPlanspecPrompt({ intent, appSlug, seamHook, maxStories = MAX_STORIES, brownfield = false }) {
  ...
  return [
    ...
    ``,
    `# WALKING SKELETON — app-surface ownership (non-negotiable, applies to BOTH shapes)`,
    `Exactly ONE story owns the APP SURFACE: the app feature/entry file(s) that mount the`,
    `verifiability seam (${hookName}, the ONLY thing that publishes \`window.__harness\`),`,
    `the app page/entry, the state/reducer COMPOSITION file, and the run loop. That owner`,
    `is the FOUNDATION, and it is a WALKING SKELETON — it BOOTS LIVE from the very`,
    `first batch. The foundation creates the frozen contract AND the one app feature that`,
    `mounts the seam, composes the reducer (importing each capability module by its`,
    `conventional path), and runs the loop — AND it creates each capability module as a`,
    `TRIVIAL STUB (an identity/no-op that satisfies the signature) so the WHOLE app`,
    `COMPILES and BOOTS with the seam live BEFORE any capability lands.`,
    `The foundation's \`touches\` MUST COVER every file the foundation itself writes: the`,
    `app feature/seam/page/composition/loop AND every capability STUB module path — either`,
    `list each stub path, or declare the stub directory as a glob (e.g. \`src/<sliceDir>/**\`).`,
    `This is load-bearing, not bookkeeping: a stub path the foundation must create but`,
    `OMITS from its own \`touches\` is an out-of-scope write that the live scope gate`,
    `rejects, failing the very foundation that has to boot the app. Each stub path ALSO`,
    `appears in its owning slice's touches — that foundation↔slice overlap is legitimate`,
    `(the slice runs only AFTER the foundation and replaces the stub in place), so do NOT`,
    `drop a stub from the foundation to avoid the overlap.`,
    `EVERY capability slice is a PURE MODULE ONLY — a system/logic function, a render`,
    `helper, or a data/query module implemented against the frozen contract — that REPLACES`,
    `its own stub. A capability slice's \`touches\` MUST be its OWN module file(s) only`,
    `(plus that module's sibling test). A slice's touches MUST NOT include ANY \`*.feature.*\``,
    `file, the app page/entry, the seam file, or the composition/reducer file. The live`,
    `disjoint-touches gate physically blocks a slice from writing app surface — but YOU,`,
    `the planner, must NEVER ASSIGN app-surface files to a slice in the first place.`,
    `THIS IS THE FIX for the competing-feature collision: when a slice was allowed to own`,
    `app surface it created a SECOND feature that ALSO published the seam, two features`,
    `mounted on the page and interfered. Only the foundation owns app surface; every slice`,
    `merely fills in a pure module the foundation already imports.`,
    ``,
    ...
    `2. CONTRACT + SKELETON — design the frozen interface FIRST: the state shape`,
    `   \`snapshot()\` exposes, the action/event type union, and the exact module`,
    `   signature each capability slice will export (e.g. one reducer/system/handler`,
    `   function per slice). The FOUNDATION story creates these contract files`,
    `   COMPLETELY (no later story edits them) AND builds the WALKING SKELETON around`,
    `   them: the one app feature that mounts the seam + composes the reducer + runs the`,
    `   loop, plus a TRIVIAL STUB for every capability module, so the app boots live`,
    `   before any slice lands (see WALKING SKELETON above).`,
    `3. SLICES — one story per capability, each a PURE MODULE that REPLACES the`,
    `   foundation's stub for that capability, OWNING its own disjoint file(s), e.g.`,
    `   \`src/slices/<capability>.ts\` plus that capability's render helper. A slice`,
    `   touches ONLY its module (+ its sibling test) — NEVER a feature/page/seam/`,
    `   composition file (that is app surface the foundation owns). NEVER plan one shared`,
    `   file that every feature edits (a god reducer/store — it serializes the whole`,
    `   build). If two capabilities want the same file, split the file along the contract`,
    `   or merge those capabilities into one story.`,
    ...
    `   foundation story, NOT on each other. Because the foundation ALREADY composed the`,
    `   app and mounted the seam, there is NO separate "assemble" story that re-composes`,
    `   or re-mounts — the app is already whole and live; slices only fill it in.`,
    ...
    `   (b) does any slice's touches include a \`*.feature.*\`, the app page/entry, the`,
    `       seam file, or the composition/reducer? that is app surface — move it to the`,
    `       foundation and leave the slice a pure module.`,
    `   (b2) does the FOUNDATION's touches cover EVERY file it writes, including every`,
    `        capability stub module path? if any stub the foundation must create is`,
    `        missing from its touches, add it (or a covering glob) — the foundation's own`,
    `        stub write would otherwise be rejected as out-of-scope.`,
    `   (c) sharded only: is the graph a chain (each story depending on the previous`,
    `       one)? you invented false deps — the sharded target shape is foundation`,
    `       (walking skeleton) → ONE WIDE LAYER of independent pure-module slices, NO`,
    `       assemble. (A coherent plan's phased chain is legitimate when each edge is a`,
    `       TRUE prerequisite.)`,
    `   (d) does every user-driven capability carry a behavioral AC proving it through`,
    `       \`snapshot()\` against the already-live app? (there is no assemble story to`,
    `       defer this to.)`,
    ``,
    ...
    `- (i) ONE foundation story = the WALKING SKELETON (see above): the FROZEN CONTRACT`,
    `  (the state model, types, the action/event union, the module signature each later`,
    `  phase implements against) PLUS the one app feature that mounts the seam, composes`,
    `  the reducer, and runs the loop, PLUS a TRIVIAL STUB for every capability module —`,
    `  so the app COMPILES, BOOTS, and publishes \`window.__harness\` from day one. This`,
    `  story OWNS ALL app surface, declares the invariants (see below), and carries the`,
    `  seam-mount AC (thenObservable "snapshot.status equals 'idle'" or equivalent).`,
    `- (ii) CAPABILITY stories, one cohesive mechanic each, ordered by TRUE`,
    `  prerequisites. Each REPLACES the foundation's stub for its capability with a PURE`,
    `  MODULE and touches ONLY that module (+ its test) — never app surface. A`,
    `  dependsOn CHAIN is fine and EXPECTED here — capabilities coupled through the same`,
    `  runtime state genuinely build on each other. Stories may run in parallel ONLY when`,
    `  their touches are disjoint. Because the app is LIVE from batch 0, each capability's`,
    `  behavior is observable through \`snapshot()\` the moment its module replaces the`,
    `  stub, so each slice carries its OWN behavioral AC.`,
    `- (iii) NO redundant "assemble" story — the foundation ALREADY composed the app and`,
    `  mounted the seam, so a story that re-composes or re-mounts is redundant AND cannot`,
    `  pass RED-first (its behavioral ACs would restate foundation state that is already`,
    `  true, so the tests pass before any implementation). ONLY if cross-capability`,
    `  journeys are NOT already covered by the slice ACs, you MAY add ONE THIN final`,
    `  "end-to-end journeys" story that adds NO app surface and NO composition (touches a`,
    `  test file only) and carries ONLY cross-capability behavioral ACs against the live`,
    `  app. Prefer NO final story.`,
    ...
    ``,
    `# HARD RULES`,
    `- At most ${maxStories} stories. Use as many as the idea needs — a tiny idea is 2`,
    `  (the walking-skeleton foundation + one capability slice that replaces its stub);`,
    `  never pad, never add a redundant assemble story, and never drop a named capability.`,
    ...
    `- ATOMIC ACs (hard — the generative root of un-runnable test bindings, Incident`,
    `  C): each acceptanceCriteria entry asserts EXACTLY ONE verifiable claim. NEVER`,
    `  bundle several checks into one AC — e.g. "the config loads AND every entry has`,
    `  a unique id AND the default is applied" is THREE ACs, not one. One claim maps`,
    `  to one test file and one clean, machine-runnable testRef; a multi-claim AC`,
    `  forces the test-author to bind several files into one ref, which the completion`,
    `  gate cannot execute. If splitting a story's claims into atomic ACs would exceed`,
    `  the 6-AC budget, SPLIT the story into phased stories — never bundle claims to`,
    `  fit the budget.`,
    `- Structure (when planShape is "sharded"): 1 foundation story = the WALKING`,
    `  SKELETON (the contract files — state model + types/constants that \`snapshot()\``,
    `  exposes + the slice module signatures — PLUS the one app feature/seam/composition/`,
    `  loop AND a trivial stub per capability, all booting live), and one PURE-MODULE`,
    `  story PER core capability that REPLACES its stub. There is NO separate "assemble"`,
    `  story — the foundation already assembled and mounted the app; slices only fill in`,
    `  the modules it imports. (When planShape is "coherent", use the PHASED-COHERENT`,
    `  rules above instead — foundation walking skeleton → capability phases.)`,
    ...
    `- verify intents (label each AC by what actually PROVES it — Incident C mislabel`,
    `  class): "build" is ONLY a genuine whole-project compile/typecheck claim (the`,
    `  app tsc/builds cleanly), needsBrowser false. A property of authored DATA or a`,
    `  logic/STATE claim — a count, a reachability/path property, a uniqueness or`,
    `  seeded-shape check, a reducer/state transition — is verify:'state' (bound to a`,
    `  single unit test on that data), NOT verify:'build'. "appearance" (renders/`,
    `  looks right) · "behavior" (a driven interaction, needsBrowser true) · "manual"`,
    `  (only when nothing else fits). A runtime behavior is NEVER verified by "build",`,
    `  and a data/state property is NEVER verified by "build".`,
    `- ONE HOME per property (Incident C double-declaration): a foundation DATA`,
    `  property (reachability, uniqueness, seeded-data shape, a count that must hold)`,
    `  belongs in \`invariants\` OR in \`acceptanceCriteria\` — never BOTH. Declare`,
    `  data/schema properties as invariants; reserve acceptanceCriteria for`,
    `  behavior/interface claims. Declaring one property as both an AC and an`,
    `  invariant binds it twice with inconsistent refs and gates the story on the`,
    `  redundant one.`,
    `- SEAM WIRING (non-negotiable — QA hard-fails without it): ${hookName} is the ONLY`,
    `  thing that publishes \`window.__harness\`. The FOUNDATION (walking skeleton) story`,
    `  — and ONLY it — MUST route the live app state through that scaffold hook: a`,
    `  hand-rolled \`useReducer\`/store bypasses the seam, the harness never mounts, and`,
    `  every deployed-app QA probe fails with SEAM_NEVER_PUBLISHED. The foundation's ACs`,
    `  MUST include one asserting the seam mounts (e.g. thenObservable "snapshot.status`,
    `  equals 'idle'"). NO capability slice may mount the seam or create a competing`,
    `  feature/app entry — the seam is mounted ONCE, by the foundation; each slice`,
    `  carries its OWN behavioral AC proving its mechanic through \`snapshot()\` against`,
    `  the already-live app.`,
...
export function buildQuickPlanspecRepairPrompt({ intent, appSlug, seamHook, maxStories = MAX_STORIES, brownfield = false, stories, violations }) {
  ...
  const widthDirective = [
    ...
    `same capabilities and acceptance criteria; change the DECOMPOSITION (the FOUNDATION`,
    `is the WALKING SKELETON that owns ALL app surface — the one feature/seam/composition/`,
    `loop plus a stub per capability; every slice's touches are its OWN pure module only,`,
    `never a feature/page/seam/composition file) and the GRAPH (slices depend on the`,
    `foundation only; NO separate assemble story re-composes or re-mounts the app) so that`,
  ...
  const phaseDirective = [
    ...
    `      per the PHASED-COHERENT rules above: a WALKING-SKELETON foundation story (the`,
    `      frozen contract + the one app feature that mounts the seam + composes the loop`,
    `      + a stub per capability, booting live) → pure-module capability phases in`,
    `      true-prerequisite order that each REPLACE a stub. NO separate assemble story —`,
    `      the foundation already composed and mounted the app.`,
