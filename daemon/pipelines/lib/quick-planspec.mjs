...
export function buildQuickPlanspecPrompt({ intent, appSlug, seamHook }) {
  ...
  return [
    ...
    `buildable plan_spec — a flat, ordered list of stories a coding agent implements`,
    `one at a time. NO epics, NO waves, NO PRD/UX/architecture prose. Be concrete and`,
    ...
    ``,
    `# THINK FIRST (before writing JSON)`,
    `1. What is the CORE experience? List the distinct capabilities the operator asked`,
    `   for — every interaction, view, and rule they named. A plan that silently drops`,
    `   half of them produces a hollow, "lame" app that technically runs but does almost`,
    `   nothing. Cover the WHOLE idea; if it won't fit in ${MAX_STORIES} stories, make`,
    `   each story broader — never drop a named capability.`,
    `2. What is the app's STATE? Name the fields \`snapshot()\` should expose so behavior`,
    `   is observable (e.g. current view, selection, entities, status, derived totals).`,
    `3. For every capability the user DRIVES, what is the input → observable result? That`,
    `   pair becomes a behavioral acceptance criterion. This is the difference between an`,
    `   app that works and one that just renders.`,
    ``,
    ...
    ``,
    `# HARD RULES`,
    `- At most ${MAX_STORIES} stories, ordered foundation → features → a final`,
    `  integration/assemble story (dependencies are DERIVED from that ordering — do NOT`,
    `  emit dependsOn).`,
    `- Structure: 1 foundation story (the state model + types/constants that`,
    `  \`snapshot()\` will expose), one story PER core capability from the idea, and a`,
    `  final "Assemble the complete app" story that wires them into one working app.`,
