...
export interface QaPipelineInputs {
  ...
  acceptanceCriteria?: ReadonlyArray<{
    ...
    /** VQA v3 (E4-S2) — PM verify intent, drives the resolved oracle tier. */
    verify?: VerifyIntent;
  }>;
  /**
   * VQA v3 (E2/E4) — does this app's boilerplate ship a `window.__harness`
   * verifiability seam? Drives `state`/`behavior` ACs to the deterministic
   * L2-state oracle (vs L2-vision) and enables the oracle-strength coverage
    ...
    steps: [
      {
        ...
        command: [
          ...
          `const hasSeam = ${JSON.stringify(inputs.hasSeam ?? false)};`,
          `const report = aggregateVisualTests(tests, acs, planRigor, hasSeam);`,
          ...
          `  lines.push('### ' + c.testId + ' (' + c.classification.level + (c.classification.resolvedLevel ? ' · oracle=' + c.classification.resolvedLevel : '') + ')');`,
    ...
    steps: [
      // ── 1. qa-prepare — boot dev server, capture all screenshots ──
      {
        ...
        command: [
          ...
          // (src/app/page.tsx regenerated from src/features/*) must regenerate
          ...
          // The canvas-game scaffold's useGameStateMachine publishes
          // window.__harness iff NEXT_PUBLIC_TEST_HARNESS === '1'. Set here (QA
          // boot) and nowhere else, so probes can read state deterministically
          // while production builds stay seam-free. Harmless no-op for
          // boilerplates without a seam.
          `(NEXT_PUBLIC_TEST_HARNESS=1 nohup $QA_DEV_CMD > ${tmpResultsDir}/devserver.log 2>&1 </dev/null &)`,
          ...
          `[ "$STATUS" = "200" ] || { echo "QA_PREPARE_ERROR: server boot failed (framework=$QA_FRAMEWORK port=$QA_PORT)"; tail -40 ${tmpResultsDir}/devserver.log >&2 || true; rm -rf ${tmpResultsDir} 2>/dev/null || true; exit 1; }`,
          ...
          `QA_PORT=$QA_PORT QA_BASE_PATH=$QA_BASE_PATH node -e "$(cat <<'NODE_EOF'`,
          `const { execSync, spawn } = require('child_process');`,
          `const fs = require('fs');`,
          `const tests = ${testsJson};`,
          `const port = parseInt(process.env.QA_PORT, 10);`,
          // 2026-06-17 — prepend the dev-deploy base path (Next basePath / Vite
          // base) so QA navigates to where the app is actually served, not root
          // (which renders the framework 404 — brick1 "everything failed").
          `const basePath = process.env.QA_BASE_PATH || '';`,
          `const withBase = (u) => basePath + (u || '/');`,
          `const dir = '${tmpResultsDir}/screenshots';`,
          `const failures = [];`,
          `function runOne(t) { return new Promise((resolve) => {`,
          `  const url = 'http://localhost:' + port + withBase(t.url);`,
          ...
          `}); }`,
          // snake3 (2026-06-10) — L2 FLOW EXECUTOR. Until now flow steps were
          // parsed and the L2 judge EXPECTED labeled flow screenshots, but no
          // code ever executed the steps — every L2 flow test was structurally
          // UNCERTAIN ("image file missing"). This interpreter executes the
          // DECLARED VisualTestFlowStep spec (navigate/click/fill/wait/
          // screenshot) generically — zero app-specific logic. Playwright is
          // resolved from the app worktree's node_modules first, then the
          // global npm root (npx already proves one exists on this host).
          `let pwLib = null;`,
          `function resolvePlaywright() {`,
          `  if (pwLib) return pwLib;`,
          `  // App worktree first, then the daemon's vendored copy (rsync-daemon`,
          `  // installs it — snake3 2026-06-10), then the global npm root.`,
          `  for (const cand of ['playwright', 'playwright-core']) {`,
          `    try { pwLib = require(require.resolve(cand, { paths: [process.cwd(), '/opt/futurator-daemon'] })); return pwLib; } catch (e) {}`,
          `  }`,
          `  try {`,
          `    const g = execSync('npm root -g', { timeout: 10000 }).toString().trim();`,
          `    for (const cand of ['playwright', 'playwright-core']) {`,
          `      try { pwLib = require(g + '/' + cand); return pwLib; } catch (e) {}`,
          `    }`,
          `  } catch (e) {}`,
          `  return null;`,
          `}`,
          // VQA v3 (E5.3) — deterministic L2-state comparison operators.
          `function assertOp(op, actual, expected) {`,
          `  switch (op || 'eq') {`,
          ...
          `  if (!pw) {`,
          `    failures.push({ id: t.id, error: 'FLOW_EXEC_SKIPPED: playwright library unresolvable — captured idle frame only' });`,
          `    return runOne(t);`,
          `  }`,
          `  const vp = (t.viewport || '1280,720').replace(/x/i, ',').split(',');`,
          `  const stepLog = [];`,
          `  let browser = null;`,
          `  try {`,
          ...
          `        else if (step.action === 'click' && step.selector) { await page.click(step.selector, { timeout: 5000 }); }`,
          `        else if (step.action === 'fill' && step.selector) { await page.fill(step.selector, step.value || '', { timeout: 5000 }); }`,
          `        else if (step.action === 'wait') { await page.waitForTimeout(Math.min(step.ms || 500, 15000)); }`,
          `        else if (step.action === 'screenshot') { await page.screenshot({ path: dir + '/' + t.id + '-' + (step.label || 'shot') + '.png' }); }`,
          // ── VQA v3 (E2.2) interaction grammar dispatch — driver maps intent verbs to Playwright. ──
          `        else if (step.action === 'press') { await page.keyboard.press(step.key || 'Space'); }`,
          `        else if (step.action === 'hold') { await page.keyboard.down(step.key || 'Space'); await page.waitForTimeout(Math.min(step.ms || 200, 5000)); await page.keyboard.up(step.key || 'Space'); }`,
          `        else if (step.action === 'pointer' || step.action === 'tap') { if (step.selector) { await page.click(step.selector, { timeout: 5000 }); } else { await page.mouse.click(step.x || 0, step.y || 0); } }`,
          `        else if (step.action === 'select' && step.selector) { await page.selectOption(step.selector, step.value || ''); }`,
          `        else if (step.action === 'drag' && step.selector) { await page.dragAndDrop(step.selector, step.value || step.selector); }`,
          // page.clock (E2.3) — deterministic time; fall back to a real wait on older Playwright.
          `        else if (step.action === 'clock') { try { if (step.clockMode === 'install') { await page.clock.install(); } else if (step.clockMode === 'fastForward') { await page.clock.fastForward(step.ms || 1000); } else { await page.clock.runFor(step.ms || 1000); } } catch (ce) { await page.waitForTimeout(Math.min(step.ms || 500, 15000)); } }`,
          // H10 coverage-class actions (best-effort; assert/seed/upload/download/stroke land in E5.3 / H10 stories).
          `        else if (step.action === 'viewport') { await page.setViewportSize({ width: step.w || 1280, height: step.h || 720 }); }`,
          ...
          `    const failed = stepLog.filter(s => !s.ok);`,
          `    if (failed.length > 0) failures.push({ id: t.id, error: 'FLOW_STEP_FAILED: ' + JSON.stringify(failed).slice(0, 300) });`,
          `  } catch (e) {`,
          `    failures.push({ id: t.id, error: 'FLOW_EXEC_ERROR: ' + String(e).slice(0, 250) });`,
          `  } finally {`,
          `    try { if (browser) await browser.close(); } catch (e) {}`,
          `    try { fs.writeFileSync(dir + '/' + t.id + '-flow.json', JSON.stringify(stepLog)); } catch (e) {}`,
          `  }`,
          `}`,
          `(async () => {`,
          `  const flowTests = tests.filter(t => Array.isArray(t.flow) && t.flow.length > 0);`,
          `  const plainTests = tests.filter(t => !Array.isArray(t.flow) || t.flow.length === 0);`,
          `  const batchSize = 5;`,
          `  for (let i = 0; i < plainTests.length; i += batchSize) {`,
          `    await Promise.all(plainTests.slice(i, i + batchSize).map(runOne));`,
          `  }`,
          `  // Flow tests run SEQUENTIALLY — interactions on a shared dev server`,
          `  // must not race each other.`,
          `  for (const t of flowTests) {`,
          `    await Promise.race([runFlow(t), new Promise(r => setTimeout(r, 90000))]);`,
          `  }`,
          ...
          `  base=$(basename "$f")`,
          // snake3 forensic (2026-06-10): uploads to qa-snapshots/* failed
          // silently for the same IAM gap as review-screenshots/* — surface
          // the count so a broken gallery is diagnosable from the step log.
          `  timeout 30 aws s3 cp "$f" "s3://futurator-ai-website/${snapshotPrefix}$base" --content-type image/png > /dev/null 2>${tmpResultsDir}/s3err-last.txt || UPLOAD_FAILS=$((UPLOAD_FAILS+1))`,
          `done`,
          `if [ "$UPLOAD_FAILS" -gt 0 ]; then echo "SCREENSHOT_UPLOAD_FAILED: $UPLOAD_FAILS file(s) — $(head -c 200 ${tmpResultsDir}/s3err-last.txt 2>/dev/null)"; fi`,
      ...
      // ── 6. qa-cleanup — kill dev server ──
      {
        ...
        command: [
          ...
          `# Remove THIS run's scratch dir (screenshots already uploaded in qa-prepare,`,
          `# logs archived just above). Scoped to ${tmpResultsDir} — never a blanket`,
          `# /tmp wipe; best-effort so cleanup can never fail the run.`,
          `rm -rf ${tmpResultsDir} 2>/dev/null || true`,
          `echo "QA_CLEANUP_OK"`,
        ].join('\n'),
        timeout: 30000,
      },
