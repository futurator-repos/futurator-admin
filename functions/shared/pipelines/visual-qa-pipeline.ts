    ...
    steps: [
      ...
      // Q5.1 + Q5.2 budget enforcement matches qa-judge-l1.
      {
        ...
        command: [
          ...
          `  const prompt = ['You are a Visual QA judge for a multi-screenshot behavioral test.', 'Use the Read tool to open each of these screenshot image files in order:', allShots.map((u, i) => '  ' + (i + 1) + '. ' + u).join('\\n'), '', judgeText, '', 'NOTE: flow interactions are NOT executed — every screenshot is the page as loaded (idle, no interaction).', 'Reply on ONE line in this exact format:', 'VERDICT: PASS|FAIL|UNCERTAIN [observable|not-idle-observable] — <one-line rationale>', '', 'The bracket tag states whether the expectation describes a state these idle frames can physically show. Decide SEMANTICALLY from what the expectation needs (interaction? elapsed time? motion?), not from keywords. If not-idle-observable, the verdict must be UNCERTAIN, never FAIL.', 'Use UNCERTAIN also when an image file is missing or genuinely ambiguous.'].join('\\n');`,
          `  const child = spawn('claude', ['-p', prompt, '--model', model, '--output-format', 'text', '--allowedTools', 'Read'], { stdio: ['ignore', 'pipe', 'pipe'], timeout: wallclockMs });`,
          `  let out = '';`,
          `  child.stdout.on('data', d => { out += d.toString(); });`,
          `  let killed = false;`,
