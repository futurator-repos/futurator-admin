...
> **Status:** Waves A + B (keystone) + C (fix-loop) + D (remainder) **built, tested & committed** on `feat/pipeline-v3`; not yet deployed. Only QR-5 (a risky force-push) remains, deferred by choice. · **Created:** 2026-06-23
...
| **QAA-3 / QR-1** ✅ (Wave D)    | needs-probe routed to the honest lane AT EXECUTE — qa-judge-l1 resolves an interaction/temporal-gated test with no flow as `uncertain` (NEEDS_PROBE) before a blind idle-frame judge. Belt-and-braces with the aggregate re-bucket + the deployed CONTRACT_INCOMPLETE gate.                                                                                                                                                                                                                                                                    | `visual-qa-l2-guards.test.ts` (+1)                                    |
| **CS-2** ✅ (Wave D)            | `collectManualAcs` surfaces every `verify:'manual'` AC (with reason) in the plan-apply + import responses (`manualAcsForReview`) for operator confirmation — NO reclassification at concept (altitude rule).                                                                                                                                                                                                                                                                                                                                   | `plan-output-schema.test.ts` (+2)                                     |
| **DV-1 / DV-4** ✅ (Wave D)     | DEV build contract in the story prompt: the assembled feature MUST import+call the seam hook (publish `window.__harness`, else QA blocks SEAM_NEVER_PUBLISHED), AND render the state-gated UI each state/behavior AC describes (QA forces the state and asserts the overlay).                                                                                                                                                                                                                                                                  | prompt-steered                                                        |
| **DV-3** ✅ (Wave D)            | Declare-conform runtime half — the assert distinguishes `SNAPSHOT_KEY_DRIFT` (the app never publishes the asserted key) from a value `ASSERT_FAILED`, not a silent undefined mismatch. RULE-4 in qa-author is the author-side half.                                                                                                                                                                                                                                                                                                            | `visual-qa-l2-guards.test.ts` (+1)                                    |
| **QR-3** ✅ (Wave D)            | `waitForEvent`/`assert` over the `events[]` stream — `eventTypes` (from `events[].type/kind/name`) added to the seam eval root so `expr:'eventTypes', op:'contains'` works for string- or object-events.                                                                                                                                                                                                                                                                                                                                       | `visual-qa-l2-guards.test.ts` (+1)                                    |
| **QR-4** ✅ (Wave D)            | `seed` verb implemented via the seam `dispatch` (fails loud `SEED_UNAVAILABLE`); AND an unimplemented grammar verb now FAILS loud (`UNIMPLEMENTED_VERB`) instead of the silent `ok:true` phantom (the false-confidence root).                                                                                                                                                                                                                                                                                                                  | `visual-qa-l2-guards.test.ts` (+2)                                    |

**Remaining:** QR-5 only — split the historical commit `f8c3268` to un-bundle other-track WIP. DEFERRED on purpose: it needs a force-push on `feat/pipeline-v3` while the operator is committing concurrently, which would risk clobbering in-flight work. Do it in a quiet window with the operator’s go-ahead, not as a drive-by.
