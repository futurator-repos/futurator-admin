...
```
Story 0  (Scaffolding)
  └─► Story 1  (Constants & Types)
        ├─► Story 2  (Collision)        ─┐
        ├─► Story 3  (Particles)         │
        ├─► Story 4  (Powerups)          ├─► Story 8  (Game Loop)  ─┐
        ├─► Story 5  (Level Manager)     │                          │
        ├─► Story 7  (Renderer)         ─┘                          │
        ├─► Story 6  (Input Hook)        ──────────────────────────►├─► Story 10 (App)
        └─► Story 9  (Overlay Screens)   ──────────────────────────►┘
```

## Execution Waves (maximum parallelism)

| Wave  | Stories                      | Rationale                                                                      |
| ----- | ---------------------------- | ------------------------------------------------------------------------------ |
| **0** | `S0`                         | Foundation — everything depends on this                                        |
| **1** | `S1`                         | Shared types & constants — the contract all modules code against               |
| **2** | `S2, S3, S4, S5, S6, S7, S9` | **7 stories in parallel** — each only imports from S1, zero cross-dependencies |
| **3** | `S8`                         | Game loop — orchestrates S2+S3+S4+S5+S7, must wait for all five                |
| **4** | `S10`                        | Root app — composes S6+S8+S9                                                   |

**Critical path:** S0 → S1 → S2 (or S3/S4/S5/S7) → S8 → S10 = **5 sequential steps minimum**
