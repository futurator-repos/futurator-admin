...
| DP-E1 | **Environment provisioning isolation** — dev/staging run on their own bucket+domain (true isolation) vs shared public-bucket reserved prefixes (fallback)                       | D4    | [MECH]          | `deploy-targets` `provisioned` flag; URL host                                                     | 4=own subdomain+bucket; 🟡 shared bucket `apps/_dev/`,`apps/_staging/` prefixes; 0=collides with prod path (current: **fallback = 🟡**; subdomains = 🟢 target → F14)                                                   | 2   |
...
> subdomains (deployment-v2.5.md §14) is what flips DP-E1→🟢 and DP-L2→🟢 (F14).
...
| IE22 | **Rebuild-on-promote** (build-once violated) `[deployment]` | promotion rebuilds instead of byte-copying the tested artifact                 | copy   | —        | rebuild      | **rebuild/rung (fallback)**                     | F14 (new)        |
| IE23 | **Agent-spawn precondition missing** `[deployment]`         | agent jobs fail at spawn for a missing injected prereq (MCP config / env)      | 0      | —        | ≥1           | **MCP config absent → all spawns blocked**      | F15 (new)        |
...
> real once dev/staging get their own buckets — fallback rebuilds; **F14** = provision the
...
> self-heal in `daemon/lib/mcp-config.mjs`, owned by the `graphify`/graph agent — **F15**).
...
3. **Build-once not actually happening (DP-L2 / DP-E1 / IE22) — OPEN (F14).** Because the
...
5. **Agent-spawn precondition failure (OV11 / IE23) — OPEN (F15), cross-cutting.** pacman3's
...
/opt/futurator-daemon/mcp/mcp-config.generated.json`. The Mycelium MCP integration
   ...
   `mkdirSync`) → **F15**. Immediate unblock: daemon **Restart** (resets the latch) or
   `MYCELIUM_MCP=off`.
...
> **Hand-off note to other sessions:**
>
> - **concept-develop / pipeline owner:** two fixes need homes in `pipeline-v2.5-fixes-plan.md`
>   — **F14** (provision `dev.`/`staging.futurator.ai` subdomains so promotion byte-copies;
>   the SST recipe + EC2-IAM prereq is in deployment-v2.5.md §14) and **F15** (MCP-config
>   self-heal in `daemon/lib/mcp-config.mjs`). F15 is the highest-leverage — it's a one-file,
>   two-line robustness fix that unblocks every agent spawn.
> - **The graph/`graphify` agent:** OV11/IE23/F15 is your `mcp-config.mjs` — the `configWritten`
