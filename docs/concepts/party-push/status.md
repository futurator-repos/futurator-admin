...
| 20   | party-push-daemon             | **DONE-PENDING-DEPLOY** (2026-05-22, 16/16 stories code-complete; ship-gate is the operator-driven deploy + manual smoke) | 16 (20.1–20.16) | All 16 stories code-complete. Story 20.16 carryover (ConcurrencyManager poll-loop wiring + heartbeat CM snapshot + startup log) shipped. Test sweep: daemon 4 failed / 1584 passed (epic-dev-pipeline pre-existing); main 5 failed / 3277 passed (4 epic-dev-pipeline + 1 plan-reducer pre-existing). **Zero new regressions.** Manual gate before flipping `PARTY_PUSH_V1_ENABLED=1` (party-turn rewire) + `PARTY_PUSH_ENABLED=1` (checkpoint push): `./scripts/rsync-daemon.sh` → `sst deploy --stage production` → end-to-end smoke on `applicator` → hook adversarial smoke → PAT-rotation smoke. |
...
| 20.14 | ConcurrencyManager — unified queue + interactive-first priority | **DONE** (2026-05-22, daemon poll-loop wiring + heartbeat snapshot shipped in 20.16) | `stories/20-14-concurrency-manager.md`                  |
...
| 20.16 | Integration test sweep + deploy                                 | **CODE-COMPLETE** (2026-05-22; Tasks 1+6+7+8 done; Tasks 2-5 require operator)       | `stories/20-16-integration-deploy.md`                   |
...
- [ ] **Task — Push-enable smoke** (Story 21.2 + 21.4): in `/migrate`, flip `applicator` Push toggle ON (provide a contents:write PAT + ack branch protection). Set `PARTY_PUSH_ENABLED=1` daemon-side. Repeat checkpoint compose smoke — daemon emits `party.checkpoint.pushed` + the branch appears on GitHub.
- [ ] **Task — Open-PR smoke** (Story 22.3): in the UI checkpoint card, click "Open PR" — confirm a draft PR opens on the brownfield repo + idempotent re-open returns the same URL.
