...
The L2 recon is two complementary engines, chained deterministically by `scripts/refactor-recon/recon.mjs`:
...
2. **alias-resolver + knip = usage.** `scripts/refactor-recon/alias-resolve.mjs` recomputes the import graph from source with tsconfig `paths` resolution → trustworthy fan-in, design-system hub detection, and (∩ knip) high-confidence dead code.
...
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| **FR1** | A single command `node scripts/refactor-recon/recon.mjs <repo> [--src src] [--skip-graphify]` MUST chain graphify (AST, directed) → `knip --reporter json` → `alias-resolve.mjs` → `hotspot-detect.mjs` with zero manual steps.                                                                                                                                                                                                                                   | `recon.mjs:8`, `:45-69`                                 |
...
| **NFR15** | **Deploy-ordering observability**: the API/site half ships via `sst deploy`, the daemon half via `scripts/rsync-daemon.sh` + restart. These are independent ships; a `refactor-audit` job enqueued before the daemon is rsynced will sit `PENDING` indefinitely. The daemon (and its required runtime deps — a python with `import graphify`, and `npx knip` resolvable in the clone, plus the `scripts/refactor-recon/` directory which is **not** in the daemon repo) MUST be confirmed present on the EC2 box **before** the API endpoint is exposed. This deploy-packaging gap (recon scripts live in the parent admin repo, the daemon is rsynced separately) is an explicit prerequisite, not an implementation detail. | finding §gotchas-1,2,6; `rsync-daemon.sh`             |
...
5. **Deploy-packaging prerequisite (unsolved, must be resolved before B2 ships):** `recon.mjs` lives at repo-root `scripts/refactor-recon/`, but the daemon is rsynced from `daemon/` separately. The daemon needs a resolvable path to the recon scripts on the box. Decision required: (a) rsync `scripts/refactor-recon/` alongside the daemon, or (b) copy it into `daemon/scripts/`. EC2 runtime deps must also exist: a Python with `import graphify` (`graphifyy`) and `npx knip` resolvable in the clone.
...
| 10.2.5 | Recon is deterministic and **~0 LLM tokens** (`scripts/refactor-recon/recon.mjs` L6); the trigger copy MUST NOT imply a paid/long LLM run for the recon stage. Target perf budget surfaced to the user: recon completes in **< 3 min on a ~700-file repo** (Epic A1 AC2).                                                                                   |
...
```
node scripts/refactor-recon/recon.mjs <repo> [--src src] [--skip-graphify]
```
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| R1  | **`recon.mjs` not shipped to EC2.** It lives at repo-root `scripts/refactor-recon/`, but the daemon is rsynced separately (`rsync-daemon.sh`) from `daemon/`. The runner needs a resolvable path to recon on the box. **This is the single biggest integration risk and is not yet solved in code.**                          | High     | High       | Decide deploy-packaging in B2: either (a) `rsync-daemon.sh` also ships `scripts/refactor-recon/`, or (b) copy the scripts into `daemon/scripts/`. The runner resolves an absolute path; add a startup self-check that `recon.mjs` exists or fail fast.                                                                                       | B2         |
...
| --- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| Q1  | **How does `scripts/refactor-recon/` reach the EC2 daemon box?**                | (a) extend `rsync-daemon.sh` to ship it; (b) copy into `daemon/scripts/`; (c) git-pull the admin repo on the box                                    | (a) — single source of truth, no duplicated scripts; runner resolves an absolute path with a startup existence check                                                                                                            | **Open — blocks B2 (R1)**                          |
...
_End of PRD. Validated recon toolchain: `scripts/refactor-recon/{recon.mjs, alias-resolve.mjs, hotspot-detect.mjs, graphify-build.py}`. Epic source: `docs/epics-refactoring-module.md`._
